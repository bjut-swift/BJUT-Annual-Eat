...
def get_monthly_data(year):
    ...
    for month in month_range:
        ...
        if response and response.get('success') and 'data' in response and 'data' in response['data']:
...
def analyze_consumption(year=2024):
    ...
    # 4. 食堂消费明细 - 修改筛选条件
    canteen_consumption = (df
        .filter(pl.col('dining_place') != '其他')
    ...
    # 创建热力图数据
    heatmap_data = (df
        .filter(pl.col('category') == '饮食')  # 只看餐饮消费
    ...
    bars = ax1.bar(monthly_consumption['month'].to_list(), 
    ...
    bars = ax3.bar(canteen_consumption['dining_place'].to_list(), 
    ...
    ax3.set_xticklabels(canteen_consumption['dining_place'].to_list(), fontproperties=font)
    ...
    plt.savefig(os.path.join(output_dir, 'consumption_analysis.jpg'), dpi=300, bbox_inches='tight')
    plt.close()
